...
![Version](https://img.shields.io/badge/version-V0.26-blue)
...
- **📋 导入名单**：纯文本，每行一人，逗号后写配偶名，自动识别标题行跳过
...
右上角按钮（或 Ctrl+Shift+T）循环切换，全界面及 PDF 导出均跟随：

| 主题 | 风格 |
|------|------|
| 🌿 春日 | 暖米色背景，绿色主调，默认主题 |
| 🌙 喜夜 | 深色背景，低亮度暗红，适合夜间使用 |
| 🌸 少女 | 奶油粉底色，玫粉强调色，日系风 |
| 🌸🌙 夜樱 | 深玫粉背景，夜间粉色，少女暗夜版 |
...
> **⚠ 数据安全提示**：服务器存储为明文 JSON，请勿上传身份证号等隐私数据；**务必定期导出 JSON/CSV 自行备份**。

### ☁ 自动保存

- 管理员可在后台配置全局自动保存策略（定时 / 空闲 / 大操作触发）
- 每个场次可单独覆盖：**继承全局 / 强制开启 / 强制关闭**（三态切换）
- 首次进入时弹框询问是否开启，告知免责事项
...
├── admin.php          # 管理后台（场次管理、全局自动保存配置、密码设置）
├── dashboard.php      # 总览看板（日历视图、场次状态）
├── auth.php           # 授权码 API
├── save.php           # 保存备份到服务器（支持 ?action=setAS 场次覆盖）
├── load.php           # 从服务器加载备份
├── list.php           # 列出服务器备份
├── delete.php         # 删除备份文件
...
- 一键进入编辑页 / 删除场次
- **全局自动保存配置**：定时间隔、空闲触发、大操作触发开关
- 密码管理：修改管理/看板/全局访问三种密码
- 部署配置说明（权限、文件结构、安全提示）
...
| 版本 | 说明 |
|------|------|
| **V0.26** | 四主题（少女/夜樱）；桌名工具主题跟随；PDF 四主题配色；新用户服务器保存弹框 |
| V0.25 | 管理后台全面重构；自动保存管理器；三密码支持 |
